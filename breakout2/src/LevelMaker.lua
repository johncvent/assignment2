...
function LevelMaker.createMap(level)
    ...
    for y = 1, numRows do
        ...
        for x = 1, numCols do
            ...
            --local count = 0
            --for brick in pairs(bricks) do count = count + 1 
            --end

            --rcount = math.random(count)
            --local randbrick = bricks[rcount]

            --local randbrick = bricks[count]
            --table.remove(bricks)
            --table.insert(bricks, Brick(randbrick.x,randbrick.y,1))
    ...
    --PUT POWERUP STATUS FLAG ON A RANDOM BRICK (TO SAY THAT POWERUP SHOULD APPEAR)
    local count = 0
